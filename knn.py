...
#            dists[i_test]=np.sum(np.abs(X[i_test] - self.train_X[None,:]))
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            ...
            # find the k minimum distance
            ind = np.argpartition(dists[i], self.k)[:self.k]
            counts = np.bincount(self.train_y[ind])
            pred[i] = np.argmax(counts).astype(bool)
            pass
        return pred
